...
params <- list()
params$projectdir <- "~/shared/projects/2020_Long_Rat_TCDD/R-ODAF/"
params$species <- "rat"     # one of human, mouse, rat, hamster
params$design <- "Group"    # single experimental group of interest
params$intgroup <- c("Group","Treatment","Genotype") # Can be multiple columns of interest (covariates, etc)
params$flag <- TRUE         # runs all analysis by default when set to TRUE
params$platform <- "RNA-Seq" # Choose from RNA-Seq or TempO-Seq
params$group_facet <- NULL # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
...
params$include_only_column <- "Timepoint" # Restrict analysis to group(s) in the column listed here based on params$include_only_group.
params$include_only_group <- "5D" # Restrict analysis to this/these group(s) within the column listed in params$include_only_column
...
if(is.null(params$group_facet)){
  ...
  facets <- DESeqDesign %>% pull(params$group_facet) %>% unique()
