...
groups[[1]] <- c("group","chemical","dose")
groups[[2]] <- c("I7_Index_ID","I5_Index_ID")
groups[[3]] <- c("row","column")
groups[[4]] <- c("sample_plate")
batch_var <- "sample_plate"
treatment_var <- "chemical"
Platform <- "TempO-Seq" # Or RNA-Seq
...
plot(dendo_before_all_samples %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
...
plot(dendo_before_tech_ctrls %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
abline(h=tree_height_cutoff, col = "red", lwd = 2)
dev.off()
...
plot(dendo_before_all_exp_samples %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
abline(h=tree_height_cutoff, col = "red", lwd = 2)
dev.off()
...
for(i in seq_along(facets)) {
  samples_in_facet <- DESeqDesignExpSamples[DESeqDesignExpSamples$chemical == facets[i],
                                            "original_names"]
  ...
  #group_box <- ggplot(cor_distribution, aes(facet, mean)) + geom_boxplot() + geom_jitter()
  #print(group_box)
}
```
...
QAQC_failed_logical <- QAQC_failed_logical %>%
  ...
  dplyr::mutate(NMR=as.numeric(NMR),
                ...
                Dendogram=as.numeric(Dendogram))
...
ggpairs(QAQC_metadata_subset,
        cardinality_threshold = 24,
        columns=c(unlist(groups[[2]]),
                  "NMR_data", "Ncov5_data", "Nsig80_data", "Gini_data"),
        ggplot2::aes(colour=!!ensym(batch_var)))

ggpairs(QAQC_metadata_subset,
        cardinality_threshold = 24,
        columns=c(unlist(groups[[3]]),
                  "NMR_data", "Ncov5_data", "Nsig80_data", "Gini_data"),
        ggplot2::aes(colour=!!ensym(batch_var)))

ggpairs(QAQC_metadata_subset,
        cardinality_threshold = 24,
        columns=c(unlist(groups[[4]]),
                  "NMR_data", "Ncov5_data", "Nsig80_data", "Gini_data"),
        ggplot2::aes(colour=!!ensym(batch_var)))
...
plot(dendo_after_all_samples %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
abline(h=tree_height_cutoff, col = "red", lwd = 2)
dev.off()
...
par(cex=0.4, mar = c(15, 4, 4, 2))
plot(dendo_after_tech_ctrls %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
abline(h=tree_height_cutoff, col = "red", lwd = 2)
dev.off()
...
par(cex=0.2, mar = c(15, 4, 4, 2))
plot(dendo_after_all_exp_samples %>% dendextend::set("labels", labels_to_use),
     main = "log2 CPM",
     horiz = F)
abline(h=tree_height_cutoff, col = "red", lwd = 2)
dev.off()
...
ggplot(QAQC_annotated, aes(x=treatment_var, color=factor(dose))) +
  geom_boxplot(aes(y=Gini_data)) +
