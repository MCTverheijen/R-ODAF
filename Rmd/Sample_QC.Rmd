...
dendo_before_all_samples <- 1-cor(as.matrix(cpm), method = "spearman")
dendo_before_all_samples <- hclust(as.dist(dendo_before_all_samples), method="average")
...
tech_ctrl_names <- DESeqDesign %>% 
  dplyr::filter(technical_control==T) %>%
  dplyr::pull(original_names)

sample_names <- DESeqDesign %>% 
  dplyr::filter(technical_control==F) %>%
  dplyr::pull(original_names)
...
dendo_before_tech_ctrls <- hclust(as.dist(dendo_before_tech_ctrls), method="average")
...
dendo_before_all_exp_samples <- hclust(as.dist(dendo_before_all_exp_samples), method="average")
...
dendo_after_all_samples <- 1-cor(as.matrix(cpm %>% dplyr::select(-all_of(outliers))),
                                  method = "spearman")
dendo_after_all_samples <- hclust(as.dist(dendo_after_all_samples), method="average")
...
dendo_after_tech_ctrls <- hclust(as.dist(dendo_after_tech_ctrls), method="average")
...
dendo_after_all_exp_samples <- hclust(as.dist(dendo_after_all_exp_samples), method="average")
