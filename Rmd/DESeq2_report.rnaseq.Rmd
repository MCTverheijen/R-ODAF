...
params:
  ...
  group_filter: ["17B-estradiol"] #!r NA # Which group will this report be done on?
...
if(!is.na(params$group_facet)) {
  ...
  dplyr::filter(!!sym(params$group_facet) %in% params$group_filter) %>%
  pull(params$design) %>% 
  unique()
		...
		for (gene in 1:nrow(dds)) { # Loop over each gene
			...
			for (group in 1:length(SampPerGroup)) { # Test if group passes
				...
				if (Check == FALSE) {checkSpike <- c(checkSpike, Check)} else {
					checkSpike <- c(checkSpike,
					                ((max(Counts[gene, sampleCols]) - median(Counts[gene,sampleCols])) >=
