...
params:
  ...
  intgroup: ["group","batch"]           # experimental group of interest plus covariates; can be more than one
  nuisance: "batch" # To remove batch effects for PCA/Heatmaps visualiziations
  flag: TRUE                  # runs all analysis by default when set to TRUE
  platform: "TempO-Seq"         # RNA-Seq or TempO-Seq
  group_facet: "chemical" #!r NA          # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
  group_filter: "BPA" #!r NA          # Which group will this report be done on?
...
subtitle: "`r paste(params$platform, 'analysis', ifelse(!is.null(params$group_filter), paste('for', params$group_filter), ''))`"
...
if(species=="human"){
  ...
  kegg_organism <- "hsa"
  temposeq_manifest <- "191004_Human_Whole_Transcriptome_2.0_Manifest.txt" # options: "191113_Human_S1500_Surrogate_2.0_Manifest.csv" "191004_Human_Whole_Transcriptome_2.0_Manifest.txt"
} else {
    if(species=="mouse"){
      # Mouse:
...
# Not necessary by default but might be useful.
...
#  contrasts <- contrasts # figure out how you want to reorder
#}
  ...
  if (!identical(as.data.frame(round(counts(dds))),
    ...
    if(file.exists(file.path(paths$RData, paste0("dds_", params$group_filter, ".RData"))) & !is.na(params$group_facet) & params$use_cached_RData == TRUE) {
      ...
      if(is.na(params$group_facet)) {
        ...
        save(dds,
             file=file.path(paths$RData, paste0("dds_", params$group_filter, ".RData")))
      }
    }
}
...
if(!is.na(params$nuisance)){
  ...
  condition <- formula(paste0(paste0("~"),
                              paste0(intgroup[!intgroup %in% nuisance],
                                     collapse = " + ")))
...
smpInfo = DESeqDesign
temp4plot = data.frame(reads=round(colSums(sampleData),3))
temp4plot$original_names <- row.names(temp4plot)

temp4plot_annotated <- dplyr::left_join(smpInfo, temp4plot)

ggplot(temp4plot_annotated,
...
for (x in 1:nrow(contrasts)) {	## for all comparisons to be done	
  ...
  condition1 <- contrasts[x,2]
	...
	res <- DESeq2::results(dds[idx],
	               parallel = TRUE, BPPARAM=MulticoreParam(41),
...
ensembl <- useMart("ensembl", dataset = ensembl_species,  host = "uswest.ensembl.org")
...
genes_for_heatmap[["external_gene_name"]] <- ifelse(is.na(genes_for_heatmap[["external_gene_name"]]),
               genes_for_heatmap[["ensembl_gene_id"]],
               genes_for_heatmap[["external_gene_name"]])

genes_for_heatmap[["external_gene_name"]] <- ifelse(genes_for_heatmap[["external_gene_name"]]=="",
               genes_for_heatmap[["ensembl_gene_id"]],
               genes_for_heatmap[["external_gene_name"]])

genes_for_heatmap <- genes_for_heatmap %>% pull(external_gene_name) %>% unique()
...
for (i in seq_along(resList)) {
  ...
  DESeq2::plotMA(resList[[i]],
                 alpha = metadata(resList[[i]])$alpha,
  ...
  if(!is.na(params$group_facet)) {
    write.table(bmdexpress,
                file=file.path(paths$DEG_output,
                               paste0("bmdexpress_input_", params$group_filter, ".txt")),
                quote = F,
                sep = "\t",
                row.names = F,
                col.names = T)
