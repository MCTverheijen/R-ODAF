...
params:
  ...
  group_filter: "Bis (4-chorophenyl) Sulfone"           # Which group will this report be done on?
...
if (nrow(enrich_go_mf@result %>% filter(p.adjust < 0.01 & qvalue < 0.05)) > 0) {
  ...
  print(plot1)
  if(nrow(simplified_mf) > 1) {print(plot2)} else {print("Network plot not shown due to a limited number of enriched terms.")}
  print(plot3)
  print(plot4)
} else { print("No significantly enriched terms using criteria selected") }
```
