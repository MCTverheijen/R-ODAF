...
params:
  ...
  group_facet: NULL          # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
  group_filter: NULL           # Which group will this report be done on?
  exclude_samples: ["SRR5890440","SRR5890376"]  # Optionally, a vector of sample names to exclude from the analysis
  exclude_groups: "f2" # Optionally, a vector of groups to exclude from the analysis. By default this is assumed to be in the column specified by params$design.
  include_only_column: "group" # Restrict analysis to group(s) in the column listed here based on params$include_only_group.
  include_only_group: ["male_0","male_2"] # Restrict analysis to this/these group(s) within the column listed in params$include_only_column
...
# Note that these should only be run if the repsective variables are set.
if(!is.null(params$group_facet)) {
  DESeqDesign <- DESeqDesign %>% dplyr::filter(!!sym(params$group_facet)==params$group_filter)
...
if(!is.null(params$include_only_column) & !is.null(params$include_only_group)) {
  DESeqDesign <- DESeqDesign %>% dplyr::filter((!!sym(params$include_only_column)) %in% params$include_only_group)
  contrasts <- contrasts %>% dplyr::filter(V1 %in% params$include_only_group)
}
...
if (Platform=="RNA-Seq"){
  ...
  linear_fc_filter = 1.5
  # ADD MORE CODE HERE - NEED TO CONDITIONALLY SUM PROBES IN SAMPLE DATA!
  ...
  sampleData <- sampleData %>%
    dplyr::left_join(biospyder %>% dplyr::select(GENE_SYMBOL, PROBE_NAME)) %>%
...
The code above (not shown by default) loads user-provided meta data (i.e., information about your experiment, also known as colData, or, column data). This also imports the count matrix (i.e., a table of observed counts in which each sample is a column and genes are rows).
...
if(!is.null(params$group)) { DESeqDesign[params$group] <- as.factor(DESeqDesign[params$group]) }

if (file.exists(file.path(paths$RData, "dds.RData")) & is.null(params$group_facet) & params$use_cached_RData == TRUE) {
  ...
  if (!identical(as.data.frame(round(counts(dds))),
    ...
    if(file.exists(file.path(paths$RData, paste0("dds_", params$group_filter, ".RData"))) & !is.null(params$group_facet) & params$use_cached_RData == TRUE) {
...
## Sample data (metadata about your experiment) {.tabset}
...
if(any(grepl(x=colnames(DESeqDesign), pattern="dose", ignore.case = T))){
  ...
  knitr::kable(DESeqDesign %>% group_by(!!sym(params$design)) %>% arrange(doseCol),
             row.names =  F,
...
# Conditionally sort by dose if that column name is present?
knitr::kable(DESeqDesignAsRead %>% arrange(!!sym(params$design)) %>% dplyr::filter(original_names %in% removed),
             row.names =  F,
...
for (x in 1:nrow(contrasts)) {	## for all comparisons to be done	
  ...
  condition1 <- contrasts[x,2]
	...
	print("AND")
	print("Detecting iousious spikes: Max-Median > Sum/(Rep+1)" )
...
addWorksheet(wb5, "Counts per million")
...
writeDataTable(wb5,
               ...
               keepNA = T,
               na.string = "NA")
setColWidths(wb5, sheet = 1, cols = 1:ncol(CPMddsDF), widths = "auto")
...
###########################################################################################
# Maybe run this code conditional on whether there is a column matching the word dose in the metadata
# # Create input file...
...
write.table(bmdexpress, file=file.path(paths$DEG_output, "bmdexpress_input.txt"), quote = F, sep = "\t", row.names = F, col.names = T)
...
fastbmd <- rbind( Dose=c("#CLASS:DOSE",format(DESeqDesign[DESeqDesign$original_names %in% colnames(fastbmd),]$dose, scientific=F)),
                  fastbmd,
                  stringsAsFactors=F)
write.table(fastbmd, file=file.path(paths$DEG_output, "fastbmd_input.txt"), quote = F, sep = "\t", row.names = F, col.names = T)
...
simplified_bp_for_network <- simplified_bp %>% gsfilter(by="GSSize", max=400)
...
cnetplot(simplified_bp_for_network,
         foldChange=foldChanges,
         showCategory = show10,
         cex_label_gene=0.3)

upsetplot(enrich_go_bp)
heatplot(simplified_bp, foldChange=foldChanges, showCategory = show10)
```
...
# Max size of gene set... useful for visualizing networks
simplified_mf_for_network <- simplified_mf %>% gsfilter(by="GSSize", max=400)
# I believe there is a bug in the current version of DOSE or clusterProfiler that prevents this from working
# Throws this error on plotting: Error in graph.data.frame(x, directed = FALSE) : the data frame should contain at least two columns
...
cnetplot(simplified_mf_for_network,
         foldChange=foldChanges,
         showCategory = show10,
         cex_label_gene=0.3)

upsetplot(enrich_go_mf)
heatplot(simplified_mf, foldChange=foldChanges, showCategory = show10)
```

## Cellular Component

This is not displayed by default because the cellular component ontology does not always have statistically significant enriched  genes. If you are expecting to see results here, the analysis can be repeated.  

```{r plot_GO_analysis_cc, message=FALSE, warning=FALSE, fig.width=12, collapse=TRUE}
# clusterProfiler::dotplot(enrich_go_all %>% filter(ONTOLOGY=="CC"), font.size=9, c="BgRatio")  +
#    theme(axis.text.y = element_text(angle = 0)) +
#   scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30))
...
## WikiPathways: Enrichment by DEGs within each contrast {.tabset}

This section uses clusterProfiler `enricher()` to detect enriched gene sets using [WikiPathways](https://www.wikipathways.org/) data.
...
for (i in 1:length(levels(significantResults$contrast))) {
  ...
  if(!is.null(plotList[[i]])) {print(plotList[[i]])} else {print("No pathway perturbations were detected within this contrast.")}
  cat('\n\n')
}

```

## GSEA: By contrast {.tabset}

This section uses clusterProfiler `gseKEGG()` to detect enriched KEGG pathways.

```{r 'gsea_analysis_by_contrast_make_plots', collapse=TRUE}
plotListGSEA <- list()
...
gsea_sorted <- kk %>% filter(p.adjust < .05) %>% arrange(desc(enrichmentScore))
if(dim(kk@result)[1] > 0){
  plotListGSEA[[i]] <- ridgeplot(kk)
  plotListGSEA_escore[[i]] <- clusterProfiler::gseaplot(gsea_sorted, gsea_sorted$ID[1], title=gsea_sorted$Description[1])
} else {
  plotListGSEA[[i]] <- NA
  plotListGSEA_escore[[i]] <- NA
}
...
```{r 'gsea_analysis_by_contrast_print_plots', results='asis'}
for (i in 1:length(levels(significantResults$contrast))) {
  cat("###", levels(significantResults$contrast)[i], " \n")
  if(!is.na(plotListGSEA[[i]])) {
...
## GSEA Enrichment Score Plots: By contrast {.tabset}
...
```{r 'gsea_scores_plot', results='asis'}
for (i in 1:length(levels(significantResults$contrast))) {
  cat("###", levels(significantResults$contrast)[i], " \n")
  if(!is.na(plotListGSEA_escore[[i]])) {
...
## Parameters (from the list elements for each params${whatever}) used to generate this report.

```{r paramsList, echo=FALSE}
knitr::kable(as.data.frame(unlist(params)), format="markdown")
```
