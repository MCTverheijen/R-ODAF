---
title: "Transcriptome Profiling - Rat TCDD"
...
params:
  projectdir: "~/shared/projects/2020_Long_Rat_TCDD/R-ODAF/" # Note trailing slash. We try to avoid requiring it though!
  ...
  intgroup: ["Group","Genotype","Treatment"]           # experimental group of interest plus covariates; can be more than one
  ...
  exclude_groups: ["f2","m2"] # Optionally, a vector of groups to exclude from the analysis. By default this is assumed to be in the column specified by params$design.
  include_only_column: "Timepoint" # Restrict analysis to group(s) in the column listed here based on params$include_only_group.
  include_only_group: "5D" # Restrict analysis to this/these group(s) within the column listed in params$include_only_column
...
knitr::kable(DESeqDesign %>% group_by(!!sym(params$design)) %>% tally(),
             row.names = F,
             caption="Number of samples in each experimental group") %>%
...
foldChanges <- foldChanges %>% sort() %>% rev()
foldChanges <- foldChanges[!is.na(names(foldChanges))]
if (length(foldChanges) < 10) {next}
kk <- gseKEGG(foldChanges, keyType="ncbi-geneid", organism=kegg_organism)
if(dim(kk@result)[1] > 0){plotListGSEA[[i]] <- ridgeplot(kk)} else {plotListGSEA[[i]] <- NULL}
}
```
...
for (i in 1:length(levels(significantResults$contrast))) {
  cat("###", levels(significantResults$contrast)[i], " \n")
  if(!is.null(plotListGSEA[[i]])) {print(plotListGSEA[[i]])} else {print("No pathway perturbations were detected within this contrast.")}
  cat('\n\n')
}
```

```{r 'gsego'}
# test <- clusterProfiler::gseGO(foldChanges, OrgDb = orgdb)
# gseaplot2(test)
```
