...
params:
  ...
  project_name: "Bisphenols"     # Change depending on your project name
  ...
  intgroup: ["group","dose","batch","pool"]           # experimental group of interest plus covariates; can be more than one
  ...
  group_filter: "Estradiol"           # Which group will this report be done on?
...
Following the removal of other samples (i.e., reference RNA, etc.), there were `r samples_after` samples remaining in the experiment. The samples excluded from analysis are shown in the table in the section titled "Sample data", along with complete sample metadata for the experiment.    
...
if(any(grepl(x=colnames(DESeqDesign), pattern="dose", ignore.case = T))){
  ...
  scroll_box(width = "720px", height = "720px")
} else {
...
plotPCA(rld, intgroup = params$intgroup, ntop=nrow(assay(rld))) + theme(legend.position = "bottom")
...
## Perform PCA analysis and make plot
plotPCA(rld_degs, intgroup = params$intgroup, ntop=nrow(rld_degs)) + theme(legend.position = "bottom")
...
## Perform PCA analysis and make plot
plotPCA(rld_top, intgroup = params$intgroup, ntop=nrow(rld_top)) + theme(legend.position = "bottom")
...
if (nrow(enrich_go_cc@result %>% filter(p.adjust < 0.01 & qvalue < 0.05)) > 0) {
  ...
  cnetplot(simplified_cc,
           foldChange=foldChanges,
           showCategory = show10,
           cex_label_gene=0.3)
  
  upsetplot(enrich_go_cc)
  heatplot(simplified_cc, foldChange=foldChanges, showCategory = show10)
} else { print("No significantly enriched terms using criteria selected") }
```
...
for (i in seq_along(levels(significantResults$contrast))) {
  ...
  plotList[[i]] <- clusterProfiler::dotplot(DEG_pathways, showCategory = 20, font.size=9)  +
     theme(axis.text.y = element_text(angle = 0)) +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30))
  } else { plotList[[i]] <- print("No significantly enriched terms using criteria selected") }
}
```
