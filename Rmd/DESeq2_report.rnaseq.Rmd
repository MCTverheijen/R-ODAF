...
SampleKeyFile <- file.path(paths$metadata,"metadata.fixed.txt")
...
This table shows the final list of samples that were used in the data analysis (as well as the corresponding sample information, e.g., experimental groups).  
...
cnetplot(simplified_mf, foldChange=foldChanges, showCategory = 5, cex_label_gene=0.5)
enrich_go_mf@result$Description <- stringr::str_trunc(enrich_go_mf@result$Description, width=30)
upsetplot(enrich_go_mf)
heatplot(simplified_mf, foldChange=foldChanges)
```
...
## WikiPathways: Enrichment by contrast {.tabset}
