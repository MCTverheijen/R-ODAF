...
params:
  ...
  group_facet: "sex"          # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
  group_filter: "m"           # Which group will this report be done on?
...
if(!is.null(params$exclude_groups)) {
  DESeqDesign <- DESeqDesign %>% dplyr::filter(!!sym(params$design) != params$exclude_groups)
}

# Conditionally exclude contrasts that should be filtered
...
if (file.exists(file.path(paths$DEG_output, "dds.Rdata")) & is.null(params$group_facet)) {
  ...
  save(dds, file=file.path(paths$DEG_output,"dds.Rdata")) # FIX THIS AND OTHER SAVES - when needed, modify file name to include group ID (just like HTML report output filename) ### ALSO check where bmdexpress and fastbmd text files are being saved...
  }
...
```{r print_metadata}
# Feature to add: make this section a tabset - first tab includes samples that made it to analysis; second tab is samples exlcuded; third tab is all original metadata.
...
if (file.exists("id_table.Rdata")) {
  ...
  id_table_entrez <- getBM(filters="ensembl_gene_id",
                        attributes= c("ensembl_gene_id",
                                      ...
                                      "description",
                                      "entrezgene_id",
                                      "entrezgene_accession"), # "refseq_mrna" or "refseq_peptide" may be of interest, but can't join with gene tables.
                        values=genes,
                        mart=ensembl)
...
PCAplotDEGs <- plotPCA(rld_degs, intgroup = params$intgroup, ntop=nrow(assay(rld_degs)), returnData = TRUE)
...
write.table(bmdexpress, file = "./DEG_output/bmdexpress_input.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
fastbmd <- rbind( Dose=c("#CLASS:DOSE",format(DESeqDesign[DESeqDesign$original_names %in% colnames(fastbmd),]$dose, scientific=F)),
                  fastbmd,
                  stringsAsFactors=F)
write.table(fastbmd, file = "./DEG_output/fastbmd_input.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
simplified_bp <- simplify(enrich_go_bp)
cnetplot(simplified_bp, foldChange=foldChanges)
upsetplot(enrich_go_bp)
heatplot(simplified_bp, foldChange=foldChanges)
```
...
simplified_mf <- simplify(enrich_go_mf)
cnetplot(simplified_mf, foldChange=foldChanges)
...
# Pathway Analysis using WikiPathways and GSEA (KEGG)

## WikiPathways: All DEGs
...
Wiki <- rWikiPathways::downloadPathwayArchive(organism=species_sci, format = "gmt")
wp2gene <- clusterProfiler::read.gmt(Wiki)
...
## WikiPathways: By contrast {.tabset}
...
if(dim(kk@result)[1] > 0){plotListGSEA[[i]] <- ridgeplot(kk)} else {plotListGSEA[[i]] <- "No results"}
}
```
